...
def print_formatted_data(data):
    ...
    if isinstance(data, dict):
        ...
        if "attitude" in data:
            att = data["attitude"]
            print(
                f"Attitude (Roll/Pitch/Yaw): {att.get('roll', 'N/A'):.4f}, "
        ...
        if "quaternion" in data:
            q = data["quaternion"]
            print(
                f"Quaternion (x,y,z,w): {q.get('x', 'N/A'):.4f}, "
        ...
        if "accelerometer" in data or "accel" in data:
            ...
            print(
                f"Accelerometer (x,y,z): {accel.get('x', 'N/A'):.4f}, "
        ...
        if "gyroscope" in data or "gyro" in data:
            ...
            print(
                f"Gyroscope (x,y,z): {gyro.get('x', 'N/A'):.4f}, {gyro.get('y', 'N/A'):.4f}, {gyro.get('z', 'N/A'):.4f}"
            )
        ...
        if "magnetometer" in data or "mag" in data:
            ...
            print(
                f"Magnetometer (x,y,z): {mag.get('x', 'N/A'):.4f}, {mag.get('y', 'N/A'):.4f}, {mag.get('z', 'N/A'):.4f}"
            )
        ...
        if "gps" in data or "location" in data:
            ...
            print(
                f"GPS: Lat={gps.get('latitude', 'N/A')}, "
        ...
        if "camera" in data:
            ...
            if "position" in cam:
                pos = cam["position"]
                print(
                    f"Camera Position (x,y,z): {pos.get('x', 'N/A'):.4f}, "
                    f"{pos.get('y', 'N/A'):.4f}, {pos.get('z', 'N/A'):.4f}"
                )
            if "eulerAngles" in cam:
                euler = cam["eulerAngles"]
                print(
                    f"Camera Euler (x,y,z): {euler.get('x', 'N/A'):.4f}, "
...
Examples:
  ...
  python receive_zigsim_data.py -H 0.0.0.0 -p 8888 # Explicit host and port
    ...
    parser.add_argument("-p", "--port", type=int, default=8888, help="UDP port to listen on (default: 8888)")

    parser.add_argument("-b", "--buffer-size", type=int, default=4096, help="UDP buffer size in bytes (default: 4096)")

    args = parser.parse_args()

    start_udp_server(host=args.host, port=args.port, buffer_size=args.buffer_size)
